...
(function () {
    $(document).ready(function() {
       ...
       var userNameW = $('.user-name').width();
        var userInfoNameW = $('.user-info-name').width();
        if (userNameW>=userInfoNameW) {
            userNameW = userInfoNameW - 24;
        }
       console.log(userNameW)
       $(".svgIcon").css("left",userNameW);
        //QQ登录判断
        if(!baai_user_token ){
        ...
        if ($.global_iscookie()) {
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    // 待发送商品数量渲染
                    if ( res.data.waitsend == 0 ) {
                        $('.waitsend .hint').css('display','none');
                    } else {
                        $('.waitsend .hint').css('display','block');
